...
export function RoutinesView({ onStartSession }: { onStartSession: (sessionId: string) => void }) {
  ...
  const addExercise = () => {
    ...
    setForm((f) => ({ ...f, exercises: [...f.exercises, { exerciseId: exercises[0].id, sets: 3, targetReps: 10, restSeconds: 90 }] }));
  };
  const updateExercise = (idx: number, patch: Partial<RoutineExercise>) => setForm((f) => ({ ...f, exercises: f.exercises.map((e, i) => (i === idx ? { ...e, ...patch } : e)) }));
  const removeExercise = (idx: number) => setForm((f) => ({ ...f, exercises: f.exercises.filter((_, i) => i !== idx) }));
  ...
  const startSession = async (r: Routine) => {
    const sessionExercises: SessionExercise[] = r.exercises.map((re) => ({
      ...
      sets: Array.from({ length: re.sets }, (_, i) => ({ setNumber: i + 1, reps: re.targetReps, weight: 0, completed: false })),
    }));
    const ts = now();
              ...
              <CardBody className="space-y-3 flex-1 flex flex-col">
                <div className="flex-1">
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-3">
                    {r.exercises.map((re, i) => <Badge key={i} color="gray">{exName(re.exerciseId)} · {re.sets}x{re.targetReps}</Badge>)}
                  </div>
                </div>
              ...
              <div className="space-y-2">
                {form.exercises.map((re, idx) => (
                  <div key={idx} className="flex flex-wrap items-end gap-2 p-3 rounded-xl bg-gray-50 dark:bg-gray-800/50">
                    <div className="flex-1 min-w-[140px]"><Label>Ejercicio</Label><Select value={re.exerciseId} onChange={(e) => updateExercise(idx, { exerciseId: e.target.value })}>{exercises.map((ex) => <option key={ex.id} value={ex.id}>{ex.name}</option>)}</Select></div>
                    <div className="w-16"><Label>Series</Label><Input type="number" min={1} value={re.sets} onChange={(e) => updateExercise(idx, { sets: Math.max(1, Number(e.target.value)) })} /></div>
                    <div className="w-16"><Label>Reps</Label><Input type="number" min={1} value={re.targetReps} onChange={(e) => updateExercise(idx, { targetReps: Math.max(1, Number(e.target.value)) })} /></div>
                    <div className="w-20"><Label>Descanso(s)</Label><Input type="number" min={0} value={re.restSeconds} onChange={(e) => updateExercise(idx, { restSeconds: Math.max(0, Number(e.target.value)) })} /></div>
